...
public class BannerController {
    ...
    public ResponseEntity<ResponseWrapper<Boolean>> addBanner(
        ...
        BannerRegistrationRequest bannerRegistrationRequest = objectMapper.readValue(regRequest, BannerRegistrationRequest.class);
        return ResponseEntity.ok(bannerService.registerBanner(bannerRegistrationRequest, logoFile, authorizationHeader));
    }
    ...
    public ResponseEntity<ResponseWrapper<Boolean>> Banner(
            @RequestParam(name = "file") MultipartFile logoFile,
            @Valid @RequestParam(name = "body") String regRequest,
            @PathVariable("bannerId") long bannerId, HttpServletRequest request) throws IOException {
